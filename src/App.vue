...
export default {
  ...
  methods: {
    ...
    zoomD3(id) {
      if (!this.zoomed) {
        const bubble = this.bubbles.find(bubble => bubble.id === id);
        const TR = 200;
        const FACT = TR / 74;
        const bubbleTransition = transition().duration(300);
        ...
        circle.transition(bubbleTransition).attr('r', TR);

        const paper = this.papers.find(paper => paper.bubbleId === id);
        ...
        domPaper
          ...
          .attr(
            'd',
            `M ${0} ${0} h ${0.9 * paper.w * FACT} l ${0.1 * paper.w * FACT} ${0.1
              * paper.h
              * FACT} v ${0.9 * paper.h * FACT} h ${-paper.w * FACT} v ${-paper.h * FACT}`,
          );
        domPaper
          .select('.dogear')
          .transition(bubbleTransition)
          .attr(
            'd',
            `M ${0 + 0.9 * paper.w * FACT} ${0} v ${0.1 * paper.h * FACT} h ${0.1
              * paper.w
              * FACT}`,
          );
        domPaper
